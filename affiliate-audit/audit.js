(function(){
  ...
  function providerOptions(){const select=$('providerFilter');select.innerHTML=Object.entries(PROVIDERS).map(([k,v])=>`<option value="${k}"${k==='ozmall'?' selected':''}>${v.label}</option>`).join('')+'<option value="all">すべて</option>';}
  function candidateRows(){return seed.spots.filter(s=>['A','B'].includes(s?.monetization?.affiliate_fit||'')&&providerCandidates(s).length);}
  function currentFilters(){return {q:$('textFilter').value.trim().toLowerCase(),provider:$('providerFilter').value,fit:$('fitFilter').value,status:$('statusFilter').value,inbound:$('inboundFilter')?.value||'all'};}
  ...
  function cardHtml(group){const spot=group.spot,fit=spot.monetization.affiliate_fit,providers=group.rows.map(r=>r.provider);return `<article class="audit-card"><div class="card-head"><div><span class="spot-id">${esc(spot.spot_id)}</span><h2>${esc(spot.name)}</h2><div class="place">${esc(spot.city||spot.prefecture||'')}${isInboundSpot(spot)?' · 訪日向け':''}</div></div><span class="chip fit-a">FIT ${esc(fit)}</span></div><div class="chips">${providers.map(p=>{const st=stateFor(spot.spot_id,p),status=statusFor(st,p);return `<span class="chip ${status}">${esc(PROVIDERS[p]?.label||p)} · ${statusLabel(st,p)}</span>`}).join('')}</div>${group.rows.map(r=>providerEditor(spot,r.provider,r.st)).join('')}<div class="official">公式：<a href="${esc(spot.official_url||'#')}" target="_blank" rel="noopener">${esc(spot.official_url||'未登録')}</a></div></article>`;}
