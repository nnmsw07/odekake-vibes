(function(){
  ...
  function marketFor(spot){const code=String(spot?.country_code||'').toUpperCase();if(code)return code==='JP'?'domestic':'overseas';return spot?.prefecture?'domestic':'overseas';}
  function validLink(x){return x&&(x.rawHtml||/^https:\/\//.test(String(x.url||'')));}
  function priorityFor(spot,intent){if(marketFor(spot)==='overseas'&&cfg.providerPriorityOverseas?.[intent])return cfg.providerPriorityOverseas[intent];return cfg.providerPriority?.[intent]||[];}
  function rankLink(x,intent,spot){const pri=priorityFor(spot,intent);const providerRank=pri.indexOf(x.provider);const scopeRank=x.scope==='spot'?0:x.scope==='area'?1:2;return scopeRank*100+(providerRank<0?90:providerRank);}
  ...
  function mountLinkSwitchSeeds(){
    ...
    Object.entries(cfg.sourceLinks||{}).forEach(([spotId,arr])=>(arr||[]).forEach((x,index)=>{
      if(!allowed.has(x.provider)||!validLink(x))return;
  ...
  function sourceLinksFor(spot){
    ...
    return (cfg.sourceLinks?.[spot.spot_id]||[]).filter(x=>allowed.has(x.provider)&&validLink(x)).map((x,index)=>({...x,url:resolvedSourceUrl(spot.spot_id,x,index),via:'linkswitch'}));
  }
  ...
  function sectionTitle(spot){const intent=intentFor(spot);if(intent==='stay')return'この宿を予約する';if(intent==='food')return'このお店を予約する';return marketFor(spot)==='overseas'?'現地体験を予約する':'チケット・体験を予約する';}
  ...
  window.KibunAffiliate={intentFor,marketFor,priorityFor,allLinksFor,linksFor,sourceLinksFor,linkSwitchReady,mountLinkSwitchSeeds,render};
})();
