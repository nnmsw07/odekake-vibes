...
assert.ok(/^0\.20\.\d+(?:\.\d+)?$/.test(seed.metadata.version));assert.ok(seed.spots.length>=324);
...
const app=fs.readFileSync(__dirname+'/app.js','utf8'),idx=fs.readFileSync(__dirname+'/index.html','utf8');assert.ok(app.includes("['fuji','御殿場・富士']"));assert.ok(app.includes("key==='fuji'"));assert.ok(idx.includes(`${seed.spots.length}スポット`));
