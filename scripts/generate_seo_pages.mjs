...
function discoverStaticContentUrls() {
  const dirs = ['magazine', 'plans', 'guide', 'performances', 'whats-on', 'en']; const urls = [];
...
fs.writeFileSync(path.join(spotsRoot, 'routes.json'), JSON.stringify({ generated_at: generatedAt, source, routes: routeRows }, null, 2));
