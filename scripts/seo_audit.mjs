...
function targetStaticPages(){const out=[];for(const d of ['spots','magazine','plans','guide','performances','whats-on','en']){const base=path.join(ROOT,d);if(!fs.existsSync(base))continue;const stack=[base];while(stack.length){const cur=stack.pop();for(const ent of fs.readdirSync(cur,{withFileTypes:true})){const full=path.join(cur,ent.name);if(ent.isDirectory())stack.push(full);else if(ent.isFile()&&ent.name==='index.html'){const html=fs.readFileSync(full,'utf8');if(!robotsMeta(html).includes('noindex'))out.push(full)}}}}return out}
