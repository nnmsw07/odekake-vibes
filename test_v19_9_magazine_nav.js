...
assert.equal(seed.spots.length,291);assert.equal(seed.metadata.version,'0.19.9');
const names=seed.spots.slice(-5).map(s=>s.name);for(const n of ['chano-ma 横浜','chano-ma 二子玉川','chano-ma 池袋','和カフェyusoshi chano-ma 上野','和カフェyusoshi chano-ma 立川'])assert.ok(names.includes(n),n);
assert.ok(!seed.spots.some(s=>/chano-ma 代官山|chano-ma 秋葉原/.test(s.name)),'closed branches must not be added');
for(const s of seed.spots.slice(-5)){assert.equal(s.plan_profile?.meal,true);assert.equal(s.plan_profile?.family_recovery,true);assert.ok(s.media_strategy?.hero_priority?.includes('google_places'));}
const index=fs.readFileSync('index.html','utf8');assert.ok(index.includes('KIBUN MAGAZINE'));assert.ok(index.includes('mobile-bottom-nav'));assert.ok(index.includes('291スポット'));
...
console.log('V19.9 MAGAZINE PASS: 291 spots + 5 chano-ma + 6 articles + plan library + mobile nav');
