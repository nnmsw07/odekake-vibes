...
assert.strictEqual(seed.metadata.version,'0.20.8.0');
assert.strictEqual(seed.spots.length,431);
...
assert.strictEqual(new Set(ids).size,431);assert.strictEqual(new Set(slugs).size,431);assert.strictEqual(new Set(names).size,431);
...
const links=cfg.sourceLinks;assert.strictEqual(Object.keys(links).length,126,'affiliate spot coverage regression');assert.strictEqual(Object.values(links).reduce((n,a)=>n+a.length,0),136,'affiliate direct-link count regression');
...
const idx=fs.readFileSync(__dirname+'/index.html','utf8');assert(idx.includes('431スポット'));assert(idx.includes('data.js?v=2080'));assert(idx.includes('affiliate-config.js?v=2080'));
