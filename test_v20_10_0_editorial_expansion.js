...
assert.ok(seed.spots.length>=446,'expected at least 446 spots');
for(let n=432;n<=446;n++) assert.ok(seed.spots.some(s=>s.spot_id===`spot_${n}`),`missing spot_${n}`);
assert.strictEqual(Object.keys(audit.spots).length,seed.spots.length,'affiliate audit must cover all spots');
assert.ok(Object.keys(aff.sourceLinks).length>=153,'expected at least 153 affiliate-configured spots');
assert.ok(Object.values(aff.sourceLinks).flat().length>=164,'expected at least 164 direct affiliate links');
...
const home=fs.readFileSync('index.html','utf8');assert.ok(home.includes(`関東＋伊豆${seed.spots.length}スポット`),'home spot count stale');assert.ok(home.includes('magazine/outdoor-stay-with-comfort/'),'home magazine preview stale');
