...
assert(/app\.js\?v=(?:2081|2090|2091)/.test(idx));assert(/data\.js\?v=(?:2081|2082|20100)/.test(idx));assert(/styles\.css\?v=(?:2081|2083|2091)/.test(idx));
const ctx={window:{}};vm.createContext(ctx);vm.runInContext(fs.readFileSync(__dirname+'/data.js','utf8'),ctx);
const seed=ctx.window.ODEKAKE_SEED;assert.ok(['0.20.8.1','0.20.8.2','0.20.10.0'].includes(seed.metadata.version));assert.ok(seed.spots.length>=431);
